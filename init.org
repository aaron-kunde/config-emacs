...
** Use /Org mode/
*** Agenda files
The list of agenda files is specific to different environments and user accounts. Each user account has to define its own list of files in a textfile ~org-agenda-files~ and put this in the user-specific emacs configuration directory (default: ~/.emacs.d~). The file ~org-agenda-files~ should like the following example:
#+begin_example
...
* Use LSP Mode
Install [[https://emacs-lsp.github.io/lsp-mode/][LSP Mode]] with the prefix /C-c l/ for LSP commands and activate integration with [[* Use /which-key/][which-key]].

The folling [[https://emacs-lsp.github.io/lsp-mode/page/languages/][languages]] are added to be usedthe LSP mode:
- [[https://emacs-lsp.github.io/lsp-mode/page/lsp-bash/][Bash]]: Based on https://github.com/bash-lsp/bash-language-server, [[https://www.shellcheck.net/][ShellCheck]] should be installed before as a dependency.
